// core/renderer.js
const Renderer = {
  groundImg: new Image(), heroImg:new Image(),
  ...
  loadImages: function(callback){
    ...
    let loadedCount=0;
    const imgs=[this.groundImg,this.heroImg,this.bedImg,this.goddessImg,
      this.upStairsImg,this.downStairsImg,this.treasureImg,this.soulImg];

    imgs.forEach(img=>{
      img.onload=()=>{loadedCount++; if(loadedCount===imgs.length) callback();}
    });
  },
  ...
  drawMap: function(canvas,map,currentStage){
    ...
    ctx.drawImage(this.heroImg,Hero.pos.x*tileSize,Hero.pos.y*tileSize,tileSize,tileSize);
  }
};
